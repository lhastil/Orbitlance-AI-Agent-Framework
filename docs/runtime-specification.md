...
| **3. Non-responsibilities** | Never load `core/industry_playbooks/` into any runtime-accessible structure (reference-only, per the QA-03 rule). Never load `core/templates/` (meta-documents, not runtime content). Never mutate Core after loading. Never load anything project-specific. |
...
| **9. Failure Modes** | Unknown `project_id` → error surfaced to Runtime Engine, never swallowed. Malformed `config.md` → error. Project Loader reports raw completeness state only; deciding the consequence of incomplete Knowledge belongs to the Resolver, per `docs/project-configuration.md`'s Resolution Order table. |
...
| **11. Future Extension Points** | Partial-Knowledge resolution (e.g., Services + FAQ present, Portfolio not yet) as a deliberately-allowed degraded state for non-critical files. This would require the framework to first define which of the 8 Knowledge documents are critical vs. optional — currently all 8 are treated as required, and Knowledge fails closed if any is missing. Not a gap; a deliberate future refinement. |
...
> **Note on Rule 4:** this module implements the per-extension-point missing-resource behavior defined in `docs/project-configuration.md`'s Resolution Order table (Branding/Config → safe Core defaults; Integrations → capability degradation; Knowledge → fail loudly at activation, no fallback). The framework doc and this module spec are aligned — a previously-recorded contradiction between them was resolved during the Architecture Freeze Sprint (`known-issues.md` #3, Resolved).
...
| **10. Validation Rules** | Assembled output must never contain any string sourced from `core/industry_playbooks/` — enforced as a hard runtime assertion, not just a design intention. |
...
| | |
|---|---|
| **1. Purpose** | Enforce the universal Core guardrails bundle (Safety + Escalation + Compliance), plus the active project's additive Operating Constraints, at pre-flight and post-response checkpoints. |
...
| **3. Non-responsibilities** | Never composes the actual safe alternative response itself (detects/blocks only — a stricter re-generation or canned fallback is invoked separately). Never skips the post-response check to save latency/cost, under any circumstance. Never allows a project's Operating Constraints to relax a Core guardrail — constraints are additive only; on any conflict, Core's universal guardrail wins. Never reads Industry Playbooks directly (reference-only; industry rules reach the runtime via the project's Operating Constraints, authored by a human). |
...
| **10. Validation Rules** | Every block must include a specific reason (for observability and for constructing an honest, specific fallback rather than a generic one), and must record whether it was triggered by a Core guardrail or a project Operating Constraint. A project's Operating Constraints must never be applied in a way that weakens a Core guardrail — Core always wins on conflict. |
| **11. Future Extension Points** | Richer constraint expression (e.g. structured/machine-checkable constraints rather than prose) if free-text Operating Constraints prove hard to enforce reliably at scale. |
| **12. Unit Test Scenarios** | (a) pre-flight blocks an automatic-escalation-condition message without any Provider call being made (assert the Provider was never invoked); (b) post-response blocks a response containing a price absent from Knowledge; (c) post-response blocks an attempted diagnosis for a project whose Operating Constraints forbid it; (d) Guardrail Engine's own internal failure results in a blocked/escalated outcome, never pass-through; (e) an Operating Constraint attempting to permit something a Core guardrail forbids does not weaken the Core guardrail — the block still occurs. |
...
| **8. External Dependencies** | The actual third-party APIs behind each of the five tool contracts (CRM, Calendar, Email, Consultation Form, General Integrations). |
...
| **11. Future Extension Points** | New tool contract types beyond the current five (CRM, Calendar, Email, Consultation Form, General Integrations), following the same contract-plus-template pattern. |
...
| **10. Validation Rules** (what it checks) | Every Knowledge field the Template requires is present; no client-specific content pattern appears in what should be Core-shared files (defense-in-depth against a repeat of the earlier hardcoded-SLA class of bug); Config's declared industry playbook(s) actually exist in Core; Config's declared LLM provider is registered in the Provider Registry; Config's Operating Constraints are additive only and none attempts to relax a Core guardrail. |
...
| **Pipeline composition (decided)** | `handleRequest` must be implemented as an **ordered, injected list of pipeline stages**, not a single hand-written function body. Each stage wraps exactly one module call. Rationale: Runtime Engine depends on all 14 other modules, so a monolithic function body would become the file every engineer must edit simultaneously — a guaranteed merge-conflict and review bottleneck once multiple developers contribute in parallel. Composed stages let a stage be added, reordered, or modified without editing the others. Guardrail pre-flight and post-response stages are **non-removable** — the composition mechanism must not allow a configuration that omits them. |
...
| **ResolvedContext** | Fully-resolved Core+Project combination, ready for everything downstream | project_id, resolvedKnowledge, resolvedBranding, resolvedIntegrations, resolvedConfig (incl. llmProvider and operatingConstraints), knowledge_incomplete, degraded_capabilities, fallback_log | Created per project by Resolver, typically once per activation/deploy, cached; recomputed on underlying change | Resolver (sole writer); read by Prompt Assembler, Token Budget Manager, Guardrail Engine, Tool Executor, Provider Registry |
